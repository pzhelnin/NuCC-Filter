                                 ...
                                 const art::FindManyP<anab::ParticleID> &trackPIDAssn)
{
  ...
  if (particlesToTracks.find(pfp) != particlesToTracks.end())
  {
    ...
    vector<Point_t> posv_rm10 = this_track->Trajectory().Trajectory().Positions();
    vector<Vector_t> momv_rm10 = this_track->Trajectory().Trajectory().Momenta();
    vector<TrajectoryPointFlags> flgv_rm10 = this_track->Trajectory().Flags();
